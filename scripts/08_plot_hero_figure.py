...
def _plot_panel_a(ax, df, df_reanalysis):
    ...
    ax.fill_between([R_range[0], R_threshold], D_range[0], D_threshold,
                     color="#2166ac", alpha=0.07, zorder=0)
    ax.fill_between([R_threshold, R_range[1]], D_range[0], D_threshold,
                     color="#f4a582", alpha=0.07, zorder=0)
    ax.fill_between([R_range[0], R_threshold], D_threshold, D_range[1],
                     color="#92c5de", alpha=0.07, zorder=0)
    ax.fill_between([R_threshold, R_range[1]], D_threshold, D_range[1],
    ...
    # --- Quadrant labels ---
    ax.text(R_range[0] + 0.008, D_range[0] + 0.001,
            "Dry\u2013Linear", ha="left", va="bottom", fontsize=6.5,
            style="italic", color="#888888", zorder=10)
    ax.text(R_range[1] - 0.008, D_range[0] + 0.001,
            "Dry\u2013Nonlinear", ha="right", va="bottom", fontsize=6.5,
            style="italic", color="#888888", zorder=10)
    ax.text(R_range[0] + 0.008, D_range[1] - 0.001,
            "Moist\u2013Linear", ha="left", va="top", fontsize=6.5,
            style="italic", color="#888888", zorder=10)
    ax.text(R_range[1] - 0.008, D_range[1] - 0.001,
            "Moist\u2013Nonlinear", ha="right", va="top", fontsize=7,
    ...
    ax.scatter(hist["R"], hist["D"], c=COL_HIST, marker="o",
               s=28, alpha=0.45, edgecolors="white", linewidths=0.3, zorder=2,
               label="_nolegend_")

    # SSP2-4.5 end — squares, bright orange (distinct from SSP5-8.5)
    ax.scatter(ssp245_end["R"], ssp245_end["D"], c=COL_SSP245, marker="s",
               s=28, alpha=0.50, edgecolors="white", linewidths=0.3, zorder=2,
               label="_nolegend_")

    # SSP5-8.5 end — diamonds, bright red
    ax.scatter(ssp585_end["R"], ssp585_end["D"], c=COL_SSP585, marker="D",
               s=28, alpha=0.50, edgecolors="white", linewidths=0.3, zorder=2,
               label="_nolegend_")
    ...
    for label, sub, color, marker in [
        ...
        ax.errorbar(R_m, D_m, xerr=R_sem, yerr=D_sem,
    ...
    if len(mean_data) >= 2:
        ...
        ax.annotate(
            ...
            arrowprops=dict(arrowstyle="-|>", color=COL_SSP245,
                            lw=1.5, mutation_scale=11, alpha=0.7),
            zorder=5)
    if len(mean_data) >= 3:
        ...
        ax.annotate(
            ...
            xytext=(mean_data[0][1], mean_data[0][2]),
            arrowprops=dict(arrowstyle="-|>", color=COL_SSP585,
        ...
        if len(era5) > 0:
            ...
            for i, (r, d) in enumerate(zip(R_vals, D_vals, strict=False)):
                ...
                ax.scatter(r, d, c="black", marker=marker_w,
                           s=40, edgecolors="white", linewidths=0.5, zorder=8)
            if len(R_vals) >= 2:
                ax.annotate(
                    ...
                    zorder=7)
            ax.text(R_vals[-1] + 0.008, D_vals[-1] + 0.0005, "ERA5",
                    fontsize=7, fontweight="bold", va="bottom", zorder=9)

    # --- "X% of models above R = 0.30" annotation ---
    if len(ssp585_end) > 0:
        frac_above = (ssp585_end["R"] > R_threshold).mean()
        pct = int(round(frac_above * 100))
        ax.text(0.97, 0.50, f"{pct}% of SSP5-8.5\nmodels exceed\nR = {R_threshold:.2f}",
    ...
    # --- Legend ---
    handles = [
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor=COL_HIST,
                   markeredgecolor="white", markersize=6,
                   label="Historical (1980\u20132000)"),
        plt.Line2D([0], [0], marker="s", color="w", markerfacecolor=COL_SSP245,
                   markeredgecolor="white", markersize=6,
                   label="SSP2-4.5 (2080\u20132100)"),
        plt.Line2D([0], [0], marker="D", color="w", markerfacecolor=COL_SSP585,
                   markeredgecolor="white", markersize=6,
                   label="SSP5-8.5 (2080\u20132100)"),
        plt.Line2D([0], [0], marker="o", color="black", ls="-",
                   markerfacecolor="black", markersize=5, lw=1.5,
                   label="ERA5"),
    ]
    ...
    ax.set_title("(a) Regime diagram", fontsize=9, loc="left", fontweight="bold",
                 pad=8)
...
def _plot_panel_b(ax, df):
    ...
    raincloud(
        ax, data_list, positions, colors, labels,
        violin_width=0.40, violin_alpha=0.25,
        box_width=0.20, box_alpha=0.55,
        dot_offset=-0.22, dot_size=16, dot_alpha=0.50,
        whis=(5, 95), show_counts=True, refline=0.0,
    ...
    for _i, (label, data) in enumerate(zip(labels, data_list, strict=False)):
        ...
        logger.info("Panel (b) %s: median=%.1f%%, %d/%d below 0 (%.0f%%)",
                     label.replace("\n", " "), median, n_below, n, pct)
    ...
    ann = (f"SSP5-8.5 end-century:\n"
           f"median = {med:.0f}%\n"
           f"{n_below}/{n} models ({pct:.0f}%) show decline")
    ax.text(0.97, 0.97, ann, transform=ax.transAxes, fontsize=6.5,
            va="top", ha="right", fontweight="bold", linespacing=1.4,
            bbox=dict(boxstyle="round,pad=0.4", facecolor="white",
                      edgecolor="#cccccc", alpha=0.9))

    ax.set_ylabel(r"$\Delta K_s^2$ change from historical (%)", fontsize=8)
    ax.set_title(r"(b) Stationary wavenumber response ($\Delta K_s^2$)",
                 fontsize=9, loc="left", fontweight="bold", pad=8)
...
def _plot_panel_c(ax, pct_ks_decline=None):
    ...
    for x_c, title, subtitle, fc, ec in nodes:
        ...
        # Title (bold, larger)
        ax.text(x_c, node_y + 0.08, title,
                transform=ax.transAxes, ha="center", va="center",
                fontsize=7.5, fontweight="bold", zorder=4,
                linespacing=1.2)
        # Subtitle (clear, not too small)
        ax.text(x_c, node_y - 0.12, subtitle,
                transform=ax.transAxes, ha="center", va="center",
                fontsize=6, color="#333333", linespacing=1.2, zorder=4)
    ...
    for i in range(len(nodes) - 1):
        ...
        ax.annotate(
            "", xy=(x_end, node_y), xytext=(x_start, node_y),
            xycoords="axes fraction", textcoords="axes fraction",
            arrowprops=dict(
                arrowstyle="-|>", color=arrow_colors[i],
    ...
    ax.annotate(
        ...
        xycoords="axes fraction", textcoords="axes fraction",
        arrowprops=dict(
            arrowstyle="-|>", color="#cb181d", lw=1.8,
            ls="--", mutation_scale=14,
            connectionstyle="arc3,rad=0.4"),
        zorder=2)

    # Feedback label
    ax.text(0.465, y_bot - 0.28, "positive feedback",
            transform=ax.transAxes, ha="center", va="top",
            fontsize=6.5, style="italic", color="#cb181d",
...
def _add_panel_connectors(fig, ax_a, ax_b, ax_c):
    ...
    con_a = ConnectionPatch(
        xyA=(0.335, 1.0), coordsA=ax_c.transAxes,
        xyB=(0.5, 0.0), coordsB=ax_a.transAxes,
        color="#aaaaaa", lw=0.8, ls=":", zorder=0)
    fig.add_artist(con_a)

    con_b = ConnectionPatch(
        xyA=(0.575, 1.0), coordsA=ax_c.transAxes,
        xyB=(0.5, 0.0), coordsB=ax_b.transAxes,
        color="#aaaaaa", lw=0.8, ls=":", zorder=0)
...
def main():
    ...
    if rean_path.exists():
        ...
        logger.info("Loaded %d reanalysis rows from %s",
                     len(df_reanalysis), rean_path)
