...
def plot_wave_figure(df: pd.DataFrame, output_path: Path):
    ...
    hist_ks = df_ks[(df_ks["scenario"] == "historical") & (df_ks["period"] == "historical")]
    hist_Ks = hist_ks.set_index("model")["Ks_250"]

    labels_ks, data_ks, colors_ks, names_ks = _compute_pct_change(df_ks, "Ks_250", hist_Ks)
    ...
    ax.set_title(
        "(a) Stationary wave refractive index change", fontsize=9, loc="left", fontweight="bold"
    )
    ...
    for scenario, period, _label in CHANGE_GROUPS:
        ...
        if r_pts:
            ...
            ax.scatter(
                ...
                alpha=0.6,
                edgecolors="none",
                label=scatter_labels[(scenario, period)],
                zorder=2,
            )
    ...
    if len(all_r_vals) >= 5:
        ...
        ts_slope, ts_intercept, _, _ = sp_stats.theilslopes(all_dks_arr, all_r_arr)
        ...
        ann = f"Pearson r = {r_val:.2f}, {p_pear}\nSpearman \u03c1 = {rho_sp:.2f}, {p_spear}"
        ax.text(0.05, 0.05, ann, transform=ax.transAxes, fontsize=6, va="bottom", linespacing=1.4)
        logger.info(
            "Panel (b) Pearson r = %.3f, p = %.2e | Spearman rho = %.3f, p = %.2e | n = %d",
            r_val,
            p_val_pearson,
            rho_sp,
            p_val_spearman,
    ...
    ax.set_ylabel("$K_s^2$ change from historical (%)", fontsize=7.5)
    ax.set_title(
        "(b) Refractive index change vs nonlinearity", fontsize=9, loc="left", fontweight="bold"
    )
    ax.legend(fontsize=5.5, loc="upper right", framealpha=0.7, handletextpad=0.3, borderpad=0.4)
