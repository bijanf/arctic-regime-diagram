...
def plot_regime_diagram(
    ...
    if exclude_models:
        ...
        logger.info("Excluded models: %s (%d rows remaining)", exclude_models, len(df))
    ...
    for scenario in ["ssp245", "ssp585"]:
        for period_key, _period_name in [("mid_century", "mid"), ("end_century", "end")]:
    ...
    ax.set_ylabel(
        r"Diabatic number  $D = L_v \cdot \overline{P} \,/\, "
...
def _plot_group(ax, df, scenario, period, alpha=0.3, size=20):
    ...
    ax.scatter(
        sub["R"], sub["D"], c=color, marker=marker, s=size, alpha=alpha, edgecolors="none", zorder=2
    )
...
def _plot_trajectories(ax, means: pd.DataFrame, zorder=4):
    ...
    for scenario in ["ssp245", "ssp585"]:
        ...
        # Historical
        hist = means[(means["scenario"] == "historical") & (means["period"] == "historical")]
        if len(hist) > 0:
            points["historical"] = (hist.iloc[0]["R_mean"], hist.iloc[0]["D_mean"])

        for period in ["mid_century", "end_century"]:
            sub = means[(means["scenario"] == scenario) & (means["period"] == period)]
            if len(sub) > 0:
                points[period] = (sub.iloc[0]["R_mean"], sub.iloc[0]["D_mean"])
...
def _plot_reanalysis(ax, df_rean, zorder=6):
    ...
    for dataset in df_rean["dataset"].unique():
        ...
        ax.plot(R_vals, D_vals, color=style["color"], ls=style["ls"], lw=style["lw"], zorder=zorder)
...
def _plot_model_trajectories(ax, df, zorder=1):
    ...
    for model in models:
        ...
        for period in ["mid_century", "end_century"]:
            sub = df[
                (df["model"] == model) & (df["scenario"] == scenario) & (df["period"] == period)
            ]
            if len(sub) > 0:
...
def _annotate_RD_correlation(ax, df):
    ...
    logger.info("R-D correlation: r = %.3f, p = %.2e, n = %d", r_corr, p_corr, len(valid))
...
def _add_legend(ax, df_reanalysis=None):
    ...
    handles.append(
        plt.Line2D(
            ...
            markersize=6,
            label=PERIOD_LABELS["historical"],
        )
    )

    # SSP2-4.5
    for period, label_key in [("mid_century", "ssp245_mid"), ("end_century", "ssp245_end")]:
        handles.append(
            plt.Line2D(
                [0],
                [0],
    ...
    # SSP5-8.5
    for period, label_key in [("mid_century", "ssp585_mid"), ("end_century", "ssp585_end")]:
        handles.append(
            plt.Line2D(
                [0],
                [0],
