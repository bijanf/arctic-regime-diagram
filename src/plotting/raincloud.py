...
def _half_violin(ax, data, pos, color, side="right", width=0.3,
                 alpha=0.3, kde_bw="scott", exclude=None, clip=None):
    ...
    if side == "right":
        ax.fill_betweenx(y_grid, pos, pos + density, alpha=alpha,
                         color=color, lw=0, zorder=1)
        ax.plot(pos + density, y_grid, color=color, lw=0.5, alpha=0.6,
                zorder=1)
    else:
        ax.fill_betweenx(y_grid, pos - density, pos, alpha=alpha,
                         color=color, lw=0, zorder=1)
        ax.plot(pos - density, y_grid, color=color, lw=0.5, alpha=0.6,
                zorder=1)


def _box(ax, data, pos, color, width=0.15, whis=(5, 95), alpha=0.6,
         median_color="white", median_lw=1.5):
    ...
    box_x = pos - width / 2
    box_h = q75 - q25
    rect = FancyBboxPatch((box_x, q25), width, box_h,
    ...
    # Median line
    ax.plot([pos - width / 2.2, pos + width / 2.2], [median, median],
            color=median_color, lw=median_lw, solid_capstyle="round",
            zorder=4)
    ...
    # Whisker caps
    cap_w = width * 0.4
    ax.plot([pos - cap_w / 2, pos + cap_w / 2], [wlo, wlo],
            color=color, lw=0.8, zorder=2)
    ax.plot([pos - cap_w / 2, pos + cap_w / 2], [whi, whi],
            color=color, lw=0.8, zorder=2)

    return median


def _strip(ax, data, pos, color, offset=-0.15, size=12, alpha=0.5,
           seed=42, jitter_width=0.06, highlight_idx=None,
           highlight_size=22, zorder=2):
    ...
    ax.scatter(pos + offset + jitter[regular_mask], d[regular_mask],
    ...
    if highlight_idx is not None and len(highlight_idx) > 0:
        ...
        ax.scatter(pos + offset + jitter[hi], d[hi],
                   s=highlight_size, c=color, alpha=min(alpha + 0.2, 0.9),
                   edgecolors="black", linewidths=0.5, zorder=zorder + 1)


def raincloud(ax, data_list, positions, colors, labels,
              violin_width=0.3, violin_side="right", violin_alpha=0.3,
              box_width=0.15, box_alpha=0.6,
              dot_offset=-0.18, dot_size=12, dot_alpha=0.5,
              whis=(5, 95), kde_bw="scott",
              exclude_from_kde=None, show_counts=True,
              count_y=None, count_fontsize=5.5, count_color="grey",
              refline=None, refline_kw=None):
    ...
    for i, (data, pos, color) in enumerate(zip(data_list, positions, colors)):
        ...
        _half_violin(ax, d, pos, color, side=violin_side,
                     width=violin_width, alpha=violin_alpha,
                     kde_bw=kde_bw, exclude=excl)

        med = _box(ax, d, pos, color, width=box_width, whis=whis,
                   alpha=box_alpha)
        medians.append(med)

        _strip(ax, d, pos, color, offset=dot_offset, size=dot_size,
               alpha=dot_alpha)
...
def horizontal_strip(ax, data_list, positions, colors, labels,
                     box_height=0.25, dot_size=15, dot_alpha=0.5,
                     mean_size=50, seed=42):
    ...
    for data, pos, color in zip(data_list, positions, colors):
        ...
        ax.scatter(d, pos + jitter, s=dot_size, c=color, alpha=dot_alpha,
                   edgecolors="none", zorder=2)

        # Horizontal box
        rect = FancyBboxPatch((q25, pos - box_height / 2),
                              q75 - q25, box_height,
                              boxstyle="round,pad=0.01",
        ...
        # Median line
        ax.plot([med, med], [pos - box_height / 2.2, pos + box_height / 2.2],
                color="white", lw=1.5, solid_capstyle="round", zorder=4)

        # Mean diamond
        ax.scatter([mean_val], [pos], s=mean_size, c=color, marker="D",
